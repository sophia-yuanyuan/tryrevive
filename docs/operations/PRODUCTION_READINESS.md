...
1. OpenAI staging 审核与正式批准分离：`review` 的 reservation/analyze 还要求审核专用 Worker Secret，普通直连请求会在读取内容和预留额度前拒绝；Windows 客户端拒绝上传。审核报告和 production preflight 同时锁定模型与 `reasoning.effort`，production 必须明确报告 `analysisMode=approved`。
