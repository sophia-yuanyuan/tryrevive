...
import { recommendProjectDecision } from "./decision-recommendation";
...
function fieldValue(line: string): string {
  return cleanSnippet(line, 180)
    .replace(
      /^(?:(?:现在|目前)?\s*(?:卡在|卡住在?|停在|未完成|还没(?:有)?|没有完成|不知道|不确定)|(?:currently\s+)?(?:stuck|blocked|waiting|not sure|uncertain|haven't finished|have not finished))\s*/iu,
      ""
    )
    .replace(/^[：:，,。\s]+/u, "")
    .trim();
...
function nextActionFromStuck(stuckLine: string | undefined): {
  ...
  if (
    /(?:不知道|不确定|拿不准|选择|方案|don't know|not sure|uncertain|choose|option)/iu.test(
      stuckLine
    )
  ) {
    return {
  ...
  if (
    /(?:等待|缺少|求助|反馈|别人|确认|waiting|need help|permission|approval|feedback|reply|confirmation)/iu.test(
      stuckLine
    )
  ) {
    return {
...
export function inferLocalProject(request: LocalInferenceRequest): LocalInferenceResult {
  ...
  const goalLine =
    lines.find((line) =>
      /(?:目标|想要|希望|准备|申请|报名|作品|项目|考试|证书|goal|want to|hope to|apply|application|register|project|exam|certificate)/iu.test(
        line
      )
    ) ??
    lines[0] ??
  ...
  const completedLine = lines.find((line) =>
    /(?:已完成|完成了|已经|做到|写完了|做完了|上次|目前做完|completed|finished|already|last time|done)/iu.test(
      line
    )
  );
  const stuckLine = lines.find((line) =>
    /(?:卡在|卡住|停滞|未完成|没完成|还没|TODO|FIXME|困难|问题|但是|不知道|stuck|blocked|not finished|haven't|have not|issue|problem|don't know|cannot|can't|waiting)/iu.test(
      line
    )
  );
  const deadlineLine = lines.find((line) =>
    /(?:截止|ddl|deadline|due date|submission date|提交时间|报名时间|考试时间|exam date|答辩时间)/iu.test(
      line
    )
  );
  const whyLine = lines.find((line) =>
    /(?:值得|重要|因为|意义|为了|想成为|必须完成|important|because|matters|worth)/iu.test(line)
  );
  ...
  const suggestedDecision = recommendProjectDecision({
    text: lines.join(" "),
    hasConfirmedProgress: Boolean(completedLine),
    hasConcreteBlocker: Boolean(stuckLine)
  });
