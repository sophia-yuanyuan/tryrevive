...
def insert_improvement_section(doc):
    ...
    intro = target.insert_paragraph_before(
        "本节把执行摘要、跨项目一致反馈、Try Revive 专项反馈和开放问题中的建议统一映射到产品功能、优先级与验证方式；未达到行为门槛前，不以新增功能数量作为进展。"
    )
    ...
    rows = [
        ...
        ("命名与发布", "公开发布前检查 Try Revive 的域名、App Store、商标及健康/影像产品混淆风险。", "风险章节；90 天路线图", "发布前阻断项"),
...
def main():
    ...
    replacements = {
        "Try Revive 不能只做 AI 拆任务。 Goblin Tools 和 Tiimo 已经覆盖拆解、估时、排序、计时和下一步。真正值得测试的差异是：针对已经停滞至少 7 天的真实项目，保留其历史和卡点，在 10 分钟内让用户完成一个可验证的“复活动作”。":
            "Try Revive 不能只做 AI 拆任务。Goblin Tools 和 Tiimo 已经覆盖拆解、估时、排序、计时和下一步。真正值得测试的差异是：针对已经停滞至少 7 天的真实项目，保留其历史和卡点，在 10 分钟内让用户完成一个可验证的“复活动作”。首版还必须补齐继续/停止判断、建议理由、轻量证据和可选短冲刺，而不是扩展 backlog。",
        ...
        "名称 Try Revive 与其他健康/影像产品混淆；公开发布前需检查域名、App Store 和商标。":
            "名称混淆：Try Revive 可能与健康/影像产品混淆。缓解：公开发布前检查域名、App Store、商标和平台政策。",
        ...
        "开放问题":
            "开放问题与待验证决策",
        "Try Revive 的最强用户是非技术 Maker、ADHD/执行功能困难用户，还是学生创业者？三者不可同时作为首发。":
    ...
    insert_many_after(
        paragraph_by_text(doc, "名称混淆：Try Revive 可能与健康/影像产品混淆。缓解：公开发布前检查域名、App Store、商标和平台政策。"),
        [
